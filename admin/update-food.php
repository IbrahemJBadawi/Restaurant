...
if (isset($_POST['submit'])) {
    ...
    $q = "UPDATE food SET title = '$title_new', Price = '$price_new', image = '$image_name', Featured = '$featured', Active = '$Active' WHERE id = '$id'";
    $res = mysqli_query($conn, $q);
    if ($res) {
        $_SESSION['food'] = "<h4 class='success'>Food updated successfully.</h4>";
        header("location: manage-food.php");
        exit();
    } else {
        $_SESSION['food'] = "<h4 class='error'>Failed to update food.</h4>";
        header("location: manage-food.php");
        exit();
    }
}
